...
class Game:
    ...
    def display_available_districts(self, window, house_type):
        print(house_type)
        row_count = -1
        for item in self.district_objects:
            row_count += 1
            district_name = item.district_name
            tk.Button(window, text=district_name,
                      command=lambda district_n=district_name:self.make_house_in_district(district_n , house_type)).grid(row=row_count)
                # print(item.district_name)

    def make_house_in_district(self, district_name, house_type):
        if house_type == 'small':
            house_obj = HomeTemplate(1)
            house_obj.house_full = True
            self.houses.append(house_obj)
            for obj in self.district_objects:
                if obj.district_name == district_name:
                    obj.add_home_to_district(house_obj)
                    print('Houses in {}: {}'.format(obj.district_name, obj.houses_in_district))
        elif house_type == 'medium':
            pass
        elif house_type == 'large':
            pass

    def choose_district_window(self, house_type): # This function runs when the player chooses to create a new home.
        ...
        if len(districts) == 0:
            ...
            self.display_available_districts(show_districts_window, house_type)
    ...
    def display_building_options(self, window, money_label, wood_label):
        ...
        build_single_house_button = tk.Button(building_options_window, text=build_single_house_desc,
                                              command=lambda:self.choose_district_window('small'))
    ...
    def display_buttons(self, window, wood_label_var, money_label_var, root, population_label):
        ...
        town_information_desc = 'Civilization/Town Stats'
        ...
        town_information_button = tk.Button(window, text=town_information_desc,
                                            command=self.display_town_information_window)
        town_information_button.grid(row=9)

    def display_town_information_window(self):
        # Displays the window for the player's current game information
        window_title = '{} - Stats'.format(self.civ_name)
        population_information = 'Population: {}'.format(self.current_population)
        buildings_count_information = 'Total Buildings: {}'.format(self.total_amount_of_buildings)
        district_information = 'Total Districts: {}'.format(self.district_counter)
        town_info_window = tk.Toplevel()
        town_info_window.resizable(width=False, height=False)
        town_info_window.title(window_title)
        population_label = tk.Label(town_info_window, text=population_information)
        population_label.grid(row=0)
        buildings_label = tk.Label(town_info_window, text=buildings_count_information)
        buildings_label.grid(row=1)
        district_label = tk.Label(town_info_window, text=district_information)
        district_label.grid(row=2)
...
class District: # Class for making a new district.
    ...
    def add_home_to_district(self, house_obj):
        self.houses_in_district.append(house_obj)
